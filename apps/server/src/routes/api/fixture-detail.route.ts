...
import {
  getFixtureDetail,
  getMyPredictionsForFixture,
} from "../../services/api/fixtures/fixture-detail";
...
const fixtureDetailRoutes: FastifyPluginAsync = async (fastify) => {
  ...
  /**
   * GET /fixtures/:id/my-predictions
   *
   * - Requires auth + onboarding completion.
   * - Returns current user's predictions for this fixture in all joined groups.
   */
  fastify.get<{
    Params: { id: string };
  }>(
    "/fixtures/:id/my-predictions",
    {
      preHandler: [fastify.userAuth.requireOnboardingComplete],
    },
    async (req, reply) => {
      ...
      const userId = req.userAuth!.user.id;
      const data = await getMyPredictionsForFixture(userId, fixtureId);
      return reply.send({
        status: "success",
        data,
      });

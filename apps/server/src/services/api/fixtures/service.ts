// fixtures/service.ts
// API service for fixtures (get upcoming fixtures).
...
export async function getUpcomingFixtures(
  ...
  let where = buildUpcomingFixturesWhere({ fromTs, toTs });

  // Add league filter if provided
  if (leagueIds.length > 0) {
    where = buildFixturesByLeaguesWhere(where, leagueIds);
  }

  // Add odds filter if requested
  if (hasOddsOnly) {
    where = buildFixturesWithOddsWhere(where);
  }

...
import { useTeamOrderSettings, useUpdateTeamOrderSettings } from "@/hooks/use-settings";
...
function SortableTeamItem({ team, index, onRemove }: { team: Team; index: number; onRemove: (id: number) => void }) {
  ...
  return (
    ...
    >
      ...
      {team.imagePath && <img src={team.imagePath} alt="" className="h-5 w-5 rounded object-contain" />}
      <div className="flex-1 min-w-0">
        <p className="text-sm truncate">{team.name}</p>
      </div>
      <Button variant="ghost" size="icon" className="h-6 w-6 text-muted-foreground hover:text-destructive" onClick={() => onRemove(team.id)}>
...
function AvailableTeamItem({ team, isSelected, onToggle }: { team: Team; isSelected: boolean; onToggle: (team: Team) => void }) {
  return (
    <button
...
export default function TeamOrderSettingsPage() {
  const { data: settingsData, isLoading: settingsLoading } = useTeamOrderSettings();
  ...
  const displayed = teamsData?.data?.length ?? 0;
  const total = teamsData?.pagination?.totalItems ?? 0;

  const availableTeams = React.useMemo(() => {
    return (teamsData?.data ?? []).map((t) => ({
      id: t.id,
      name: t.name,
      imagePath: t.imagePath,
      country: t.country ? { id: t.country.id, name: t.country.name } : null,
    }));
  }, [teamsData]);

  const selectedIds = settingsData?.data?.defaultTeamOrder ?? [];
  const { data: selectedTeamsData } = useQuery({
  ...
  React.useEffect(() => {
    if (settingsData?.data?.defaultTeamOrder && selectedTeamsData?.data) {
      const teamMap = new Map(
        selectedTeamsData.data.map((t) => [t.id, { id: t.id, name: t.name, imagePath: t.imagePath, country: t.country ? { id: t.country.id, name: t.country.name } : null }])
      );
  ...
  const handleToggle = (team: Team) => {
    if (selectedIdSet.has(team.id)) {
      setSelectedTeams((items) => items.filter((i) => i.id !== team.id));
    } else {
    ...
    <div className="w-full p-4 pb-8 sm:p-6">
      <div className="max-w-5xl mx-auto space-y-4">
        <div className="flex items-center justify-between gap-3">
          <h1 className="text-xl font-semibold">Team Order</h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
          ...
          <Card>
            <CardHeader className="py-3 px-4">
              <CardTitle className="text-sm font-medium">Selected ({selectedTeams.length})</CardTitle>
            </CardHeader>
            <CardContent className="px-4 pb-4">
              {selectedTeams.length === 0 ? (
                <p className="text-center py-6 text-sm text-muted-foreground">No teams selected</p>
              ) : (
                  ...
                  <SortableContext items={selectedTeams.map((t) => t.id)} strategy={verticalListSortingStrategy}>
                    <div className="space-y-1.5 max-h-[60vh] lg:max-h-[calc(100vh-280px)] overflow-y-auto">
                      {selectedTeams.map((team, index) => (
                        <SortableTeamItem key={team.id} team={team} index={index} onRemove={handleRemove} />
                      ))}
                    </div>
                  </SortableContext>
                </DndContext>

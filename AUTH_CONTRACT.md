...
## 11. Onboarding Gate Contract

- **Server source of truth**: The server is the only authority for `onboardingRequired`.
- **/auth/me contract**: `/auth/me` **MUST** return a boolean `onboardingRequired` field computed server-side via `isOnboardingRequired()`.
- **Protected endpoint behavior**:
  - When `onboardingRequired === true`, protected endpoints **MUST** return HTTP 403 with error code `ONBOARDING_REQUIRED`.
  - When `onboardingRequired === false`, protected endpoints **MUST** behave as normal.
- **Client logout behavior**:
  - `ONBOARDING_REQUIRED` errors **MUST NOT** trigger logout.
  - `ONBOARDING_REQUIRED` errors **MUST NOT** clear tokens.
- **Client routing behavior**:
  - When `status === "authed"` and `user.onboardingRequired === true`, the client **MUST** route the user into the onboarding flow.
  - When `status === "authed"` and `user.onboardingRequired === false`, the client **MUST** allow access to protected routes.
